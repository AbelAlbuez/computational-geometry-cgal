...
Lehmann \textit{et al.}~\cite{lehmann1999} presentan un estudio
comparativo de métodos de interpolación de \emph{intensidad} en
imágenes médicas (vecino más cercano, lineal, B-splines, cúbica,
Lagrange, entre otros), concluyendo que los B-splines de alto grado
ofrecen el menor error de interpolación. Conviene distinguir ese
problema ---la interpolación de valores de intensidad entre vóxeles---
de la interpolación de \emph{contornos/forma} que aborda este trabajo,
para la cual las referencias son los métodos basados en forma
(Sección~\ref{sec:m3})~\cite{rayaudupa1990,herman1992}. La
interpolación lineal por vértices se adopta aquí como \textit{baseline}
por su simplicidad y bajo costo, antes de considerar las variantes
basadas en spline (M2) y en forma (M3).
...
Bentley--Ottmann~\cite{bentley1979} provisto por
\texttt{pujCGAL}:
...
\label{sec:m3}
M3 no mueve vértices sino la representación implícita del contorno, en la
línea de la interpolación basada en forma~\cite{rayaudupa1990}. Cada
...
\textbf{eje medio} de la forma~\cite{blum1967}. Los dos campos se
...
$1{,}0$--$1{,}15$ (p.\,ej.\ 00528-101: 97\,646 vs 87\,993~mm$^3$, razón
$1{,}11$); cuando es
...
auto-intersección. El análisis geométrico indica que,
\emph{en los pares evaluados}, los contornos ET resultaron
aproximadamente convexos y morfológicamente suaves: el promedio
ponderado de dos contornos convexos alineados por longitud de arco
tiende a producir un contorno simple. No obstante, el tumor realzante
puede presentar formas anulares o irregulares, por lo que esta
observación se limita al conjunto probado y no se generaliza a todo el
dataset. Las tres mejoras de
...
vértices por ángulo polar; Santiago: reconstrucción 3-D). La
reconstrucción 3-D presentada en este informe
(Sección~\ref{sec:recon}) se realiza con CGAL
(\texttt{poisson\_surface\_reconstruction\_delaunay}); una versión
preliminar del plan contemplaba VTK, finalmente no utilizada. Esta
decisión reduce el riesgo de integración y
...
\bibitem{lehmann1999} T.~M.~Lehmann, C.~G\"onner, and K.~Spitzer,
``Survey: interpolation methods in medical image processing,''
\textit{IEEE Transactions on Medical Imaging}, vol.~18, no.~11,
pp.~1049--1075, 1999.
...
\bibitem{bentley1979} J.~L.~Bentley and T.~A.~Ottmann, ``Algorithms
for reporting and counting geometric intersections,'' \textit{IEEE
Transactions on Computers}, vol.~C-28, no.~9, pp.~643--647, 1979.

\bibitem{blum1967} H.~Blum, ``A transformation for extracting new
descriptors of shape,'' in \textit{Models for the Perception of Speech
and Visual Form}, W.~Wathen-Dunn, Ed.\quad Cambridge, MA: MIT Press,
1967, pp.~362--380.
